d3.csv("D3_data_journalism/data/data.csv").then(rawData => {
    ...
    function generateChart(dataSet, xData, yData, xLabel, yLabel) {
        ...
        let xScale = xAxisScale(dataSet, xData);
        let yScale = yAxisScale(dataSet, yData);

        drawAxis(xScale, yScale);
        drawCircles(xScale, yScale, dataSet);
        addText(xScale, yScale, dataSet);
    ...
    function axisUpdate() {
        let xMenu = d3.select("#xdata");
        xData = xMenu.property("value");
        xLabels = xMenu.property("text");

        let yMenu = d3.select("#ydata");
        yData = yMenu.property("value");
        yLabels = yMenu.property("text");

        generateChart(rawData, xData, yData, xLabels, yLabels);
    ...
    d3.selectAll("select").on("change", axisUpdate)
});
